...
readonly MIRROR='https://mirror.sjtu.edu.cn/archlinux/$repo/os/$arch'
...
function create_image() {
  ...
  cp -r "${ORIG_PWD}/packages" "${MOUNT}/"
  arch-chroot "${MOUNT}" /usr/bin/pacman -U --noconfirm /packages/hadoop-3.3.1-1-x86_64.pkg.pacman /packages/apache-spark-3.2.0-0-any.pkg.pacman
  rm -r "${MOUNT}/packages"
